...
async def upload_file(file: UploadFile = File(...)):
    """
    Upload and process a file (PDF, DOCX, TXT, CSV).
    
    - Saves the file to disk
    - For PDF/DOCX/TXT: checks/creates embeddings
    - For CSV: registers file for CSV Agent usage (no embeddings)
    ...
    try:
        ...
        if doc_type == 'csv':
            # CSV files rely on the CSV agent and do not need embeddings
            embeddings_created = False
            message = (
                f"{file_name} is ready for the CSV agent. "
